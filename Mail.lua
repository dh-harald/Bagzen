function Bagzen:ScanMails()
    ...
    Bagzen.data.global[Bagzen.realmname][Bagzen.unitname].mail = {}
    if numItems > 0 then
        for i = 1, numItems do
            ...
            if hasItem ~= nil and hasItem > 0 then
                for j = 1, hasItem do
                    ...
                    if itemID then
                        ...
                        table.insert(Bagzen.data.global[Bagzen.realmname][Bagzen.unitname].mail, data)
                    end
                end
            end
        end
...
function SendMail(recipient, subject, body)
    ...
    if recipient_found == true then
        if Bagzen.data.global[Bagzen.realmname][recipient].mail == nil then
            Bagzen.data.global[Bagzen.realmname][recipient].mail = {}
        end
        ...
        table.insert(Bagzen.data.global[Bagzen.realmname][recipient].mail, data)
    end
